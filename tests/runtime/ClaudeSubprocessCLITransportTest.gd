...
func test_build_command_args_emits_include_hook_events_flag_when_enabled() -> void:
	var transport := _make_transport({"include_hook_events": true})

	var args := transport.build_command_args()

	assert_int(args.find("--include-hook-events")).is_greater_equal(0)


func test_build_command_args_omits_include_hook_events_flag_when_disabled() -> void:
	var transport := _make_transport()

	var args := transport.build_command_args()

	assert_int(args.find("--include-hook-events")).is_equal(-1)
