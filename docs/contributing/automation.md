...
The passing wrapper now also includes authenticated live external `reconnect_mcp_server()` coverage through a repo-owned Node `stdio` MCP fixture under `tools/examples/fixtures/mcp/`. That fixture uses the official MCP SDK resolved from the local Claude plugin cache, is first validated in the sibling Python SDK, and then proves a truthful pre-reconnect server entry without tool availability, sentinel removal, successful reconnect, and real post-reconnect tool execution in the local GDScript runtime. External live `toggle_mcp_server()` is intentionally not in the passing wrapper right now because the same Node fixture reproduces in the sibling Python SDK that `toggle_mcp_server(server_name, false)` can report `status = "disabled"` with no tools while the external tool still executes. That behavior is now tracked through a diagnostic-only `external_mcp_toggle_probe` mode, which records the disable snapshot, the post-disable execution outcome, and the current re-enable snapshot without promoting toggle semantics into the canonical wrapper.
...
Deterministic `rewind_files(user_message_id)` parity is delivered, but authenticated live rewind is intentionally not in the passing wrapper right now. A same-environment repro against both local GDScript and the sibling pinned Python SDK shows that the explored Bash-based flow is not yet an upstream-proven success path: rewinding to the plain-string replayed `UserMessage.uuid` resolves without restoring the file, while rewinding to the replayed top-level `tool_result` `UserMessage.uuid` returns `No file checkpoint found for this message.` Live rewind therefore remains future follow-up work in the wrapper.

Live `stop_task()` is now tracked through a diagnostic-only `stop_task_probe` mode instead of the canonical wrapper. Same-environment Python probes confirm real task messages exist, but they have not yet produced a stable upstream-backed `task_notification status = "stopped"` proof that is safe to promote into the passing wrapper. The local diagnostic mode mirrors that bounded posture: it records observed task ids, sends one real `stop_task(task_id)` request against the first observed task, and captures the resulting task notifications plus final result shape without claiming that stopping succeeded. External live toggle now follows the same pattern through `external_mcp_toggle_probe`: it records the baseline external tool availability, the post-disable `get_mcp_status()` snapshot, whether the tool still executes while nominally disabled, and the current re-enable snapshot without claiming that toggle semantics are fixed. `./tools/release/validate_live_cli.sh` now defaults to wrapper modes only, while explicit diagnostics such as `./tools/release/validate_live_cli.sh --mode stop_task_probe --mode external_mcp_toggle_probe` remain manually rerunnable.
