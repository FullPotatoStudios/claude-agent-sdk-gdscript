...
- `ClaudeQuery.query()` and `ClaudeSDKClient.query()` now accept either a `String` or `ClaudePromptStream`; when `can_use_tool` is configured, interactive `ClaudeSDKClient.query(String)` still works, but one-shot `ClaudeQuery.query(String)` continues to require `ClaudePromptStream`
