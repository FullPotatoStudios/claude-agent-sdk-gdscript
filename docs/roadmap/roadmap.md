...
- upstream-style `end_input()` timing is now modeled in the runtime/query stack for one-shot flows, but the shipped subprocess transport still cannot perform a true stdin half-close because Godot `OS.execute_with_pipe()` exposes a single read/write `stdio` `FileAccess`
...
- session-tag sanitization now matches upstream whole-string iterative Unicode normalization/composition plus stripping of format/private-use/unassigned codepoint ranges through generated local Unicode tables
...
Delivered in Phase 10AL:
- additive same-session and different-session overlap compatibility on connected clients through per-session turn tracking in `ClaudeQuerySession`, with named-session result routing bound by Claude's reported `num_turns`
...
- docs and runtime/UI coverage updated to surface same-session overlap explicitly in the runtime, adapter, and shipped panel
...
- `partially implemented`: the runtime transport contract and one-shot query stack now model upstream-style `wait_for_result_and_end_input()` timing for string prompts and streamed prompt flows, including delayed end-input when hooks or SDK MCP are active. The remaining gap is transport-specific: the shipped subprocess transport still cannot perform a true stdin half-close because Godot `OS.execute_with_pipe()` exposes a single read/write `stdio` `FileAccess`. Sources: upstream `src/claude_agent_sdk/_internal/query.py`, `tests/test_query.py`; local `addons/claude_agent_sdk/runtime/transport/claude_transport.gd`, `addons/claude_agent_sdk/runtime/transport/subprocess_cli_transport.gd`, `addons/claude_agent_sdk/runtime/protocol/query_session.gd`.
...
- `partially implemented`: `control_cancel_request` now marks local hook and `can_use_tool` callback contexts with a cooperative abort signal, suppresses late responses, and skips callbacks that have not started yet. Upstream still has stronger parity here because it can force-cancel in-flight asyncio tasks, while local GDScript cannot preempt an arbitrary awaited `Callable` once user code is already running. Sources: upstream `src/claude_agent_sdk/_internal/query.py`, `tests/test_query.py`; local `addons/claude_agent_sdk/runtime/claude_abort_signal.gd`, `addons/claude_agent_sdk/runtime/protocol/query_session.gd`, `tests/runtime/ClaudeQuerySessionTest.gd`.
- `delivered`: connected local clients now support overlapping turns across repeated or different `session_id` values through per-session turn tracking and additive session-scoped response streams, while `receive_response()` remains a shared global convenience stream that stops on the first `ResultMessage` seen after subscription. Named same-session result routing now keys off Claude's reported `num_turns`, while the panel transcript still renders arrival order on the shared live stream. This now matches the upstream shared-stream, no-global-busy-guard client more closely without claiming explicit upstream same-session overlap tests. Sources: upstream `src/claude_agent_sdk/client.py`, `tests/test_streaming_client.py`; local `addons/claude_agent_sdk/runtime/protocol/query_session.gd`, `addons/claude_agent_sdk/runtime/claude_sdk_client.gd`, `addons/claude_agent_sdk/runtime/adapters/claude_client_adapter.gd`, `addons/claude_agent_sdk/runtime/adapters/claude_client_node.gd`, `addons/claude_agent_sdk/ui/claude_chat_panel.gd`, `tests/runtime/ClaudeQuerySessionTest.gd`, `tests/runtime/ClaudeSDKClientTest.gd`, `tests/runtime/ClaudeClientAdapterTest.gd`, `tests/runtime/ClaudeClientNodeTest.gd`, `tests/ui/ClaudeChatPanelTest.gd`.
...
- `delivered`: session-tag sanitization now matches upstream `_sanitize_unicode()` whole-string iterative Unicode normalization/composition behavior, including decomposed combining-mark recomposition, Hangul Jamo composition, and iterative stripping of format/private-use/unassigned codepoint ranges through generated local Unicode tables. Sources: upstream `src/claude_agent_sdk/_internal/session_mutations.py`, `tests/test_session_mutations.py`; local `addons/claude_agent_sdk/runtime/sessions/claude_sessions.gd`, `addons/claude_agent_sdk/runtime/sessions/claude_unicode_sanitizer.gd`, `tests/runtime/ClaudeSessionsTest.gd`, `docs/contributing/session-history.md`.

### Validation and docs follow-ups

- `partially implemented`: extend scripted live-CLI parity validation beyond `baseline`, `structured`, and `partial` so the delivered post-v1 slices have real-authenticated coverage too. The current bounded slice now covers SDK-defined agents, local plugin discovery through `ClaudeAgentOptions.plugins`, transport-first resumed-session forking through a real-CLI `resume + fork_session=true` smoke, default `setting_sources`, explicit `["user", "project", "local"]` settings loading, filesystem agents loaded from `.claude/agents/` with `setting_sources = ["project"]`, debug-stderr callback capture, `PreToolUse` hook callbacks for real Bash tool use, interactive `can_use_tool` permission callbacks for a real Bash `touch` flow outside the project cwd, same-user `ClaudeAgentOptions.user` relaunch on POSIX through authenticated `user_current_user_auth` and `user_current_user_baseline` modes, dynamic `set_permission_mode()` / `set_model()` / `interrupt()` control on connected clients, live `get_context_usage()` diagnostics after a real turn, targeted `get_mcp_status()` diagnostics for a configured SDK MCP server, and authenticated live external `reconnect_mcp_server()` coverage through a repo-owned Node `stdio` MCP fixture that uses the official MCP SDK resolved from the local Claude plugin cache and restores tool availability after an observed pre-reconnect server entry without tools. The live plugin mode now uses the shipped `demo-plugin` fixture with repo-root `cwd` and accepts either init plugin metadata (`demo-plugin`) or init command exposure (`greet`) as truthful CLI-side discovery evidence, without claiming that slash-command execution is proven in every environment. The live `fork_session_resume` mode is intentionally bounded to externally observable CLI behavior: it proves that a resumed session with `fork_session = true` returns a new runtime session id and writes a new saved-session file while preserving the source session, without claiming that the CLI-authored forked file mirrors the deeper deterministic JSONL structure produced by local `ClaudeSessions.fork_session()`. The live `user` modes resolve the current username with `id -un`, then `whoami`, then environment fallback, require a real `sudo -n -u <current-user> /usr/bin/true` preflight, and truthfully validate only the local POSIX same-user wrapper path rather than arbitrary cross-user support. Deterministic runtime coverage still proves `rewind_files(user_message_id)` parity, but authenticated live rewind remains outside the passing wrapper because a same-environment repro against both local GDScript and the sibling pinned Python SDK shows that the explored checkpointed Bash flow is not yet an upstream-proven success path: rewinding to the plain-string replayed `UserMessage.uuid` resolves without restoring the file, while rewinding to the replayed top-level `tool_result` `UserMessage.uuid` returns `No file checkpoint found for this message.` The local `set_model()` control path now mirrors upstream `set_model(None)` reset semantics by sending `{"subtype": "set_model","model": null}` for nullable and zero-arg resets, and deterministic runtime coverage now also proves `reconnect_mcp_server()` / `toggle_mcp_server()` request-shape parity on the client, adapter, and node surfaces. The full authenticated wrapper succeeds through the current bounded live-validation slice, while `validate_live_cli.sh` defaults to wrapper modes only and keeps diagnostic-only probes opt-in through explicit `--mode` filters. New `stop_task_probe` and `external_mcp_toggle_probe` modes record real task-control and external-toggle behavior without promoting either into the passing wrapper: `stop_task_probe` captures real task ids, one live `stop_task(task_id)` request, and the resulting notification/result shape without claiming passing stop parity, while `external_mcp_toggle_probe` captures baseline external tool execution, the post-disable `get_mcp_status()` snapshot, whether execution still occurs while the server reports disabled, and the current re-enable snapshot. Same-environment Python investigation still shows no stable upstream-backed `task_notification status = "stopped"` proof yet, and the same repo-owned external Node fixture still reproduces the key disable-leg mismatch where `status = "disabled"` plus no tools can coexist with real external tool execution. Remaining priorities are live `rewind_files()` wrapper coverage, truthful live `stop_task()` wrapper coverage once upstream proves a stable stopped flow, plus wrapper-safe external-MCP live toggle coverage only after the upstream disable semantics become truthful. Sources: upstream `e2e-tests/test_agents_and_settings.py`, `e2e-tests/test_dynamic_control.py`, `e2e-tests/test_hook_events.py`, `e2e-tests/test_hooks.py`, `e2e-tests/test_sdk_mcp_tools.py`, `e2e-tests/test_stderr_callback.py`, `e2e-tests/test_tool_permissions.py`, `tests/test_streaming_client.py`, `examples/plugin_example.py`; local `tools/release/validate_live_cli.sh`, `docs/investigations/phase-5-validation.md`, `docs/investigations/phase-7-validation.md`, `docs/investigations/phase-8-validation.md`, `tests/runtime/ClaudeSDKClientTest.gd`, `tests/runtime/ClaudeClientAdapterTest.gd`, `tests/runtime/ClaudeClientNodeTest.gd`, `tools/examples/fixtures/mcp/external_echo_server.js`, `tools/spikes/phase5_runtime_smoke.gd`.
