...
- `partially implemented`: add explicit stdin half-close / `end_input` parity to the transport and query-session stack so one-shot `ClaudeQuery.query()` and streamed prompt flows can match upstream `wait_for_result_and_end_input()` behavior instead of keeping stdin open until full disconnect. Sources: upstream `src/claude_agent_sdk/_internal/query.py`, `tests/test_query.py`; local `addons/claude_agent_sdk/runtime/transport/claude_transport.gd`, `addons/claude_agent_sdk/runtime/transport/subprocess_cli_transport.gd`, `addons/claude_agent_sdk/runtime/protocol/query_session.gd`.
- `partially implemented`: port upstream buffered stdout parsing behavior into the local transport/query path and expose the missing `ClaudeAgentOptions.max_buffer_size` control. The current GDScript stack still assumes each emitted stdout line is one complete JSON object, so concatenated JSON objects, split/minified payloads, and stray non-JSON stdout lines such as sandbox/debug messages can fail the session instead of being buffered or skipped the way upstream now does. Sources: upstream `src/claude_agent_sdk/_internal/transport/subprocess_cli.py`, `src/claude_agent_sdk/types.py`, `tests/test_subprocess_buffering.py`; local `addons/claude_agent_sdk/runtime/claude_agent_options.gd`, `addons/claude_agent_sdk/runtime/transport/subprocess_cli_transport.gd`, `addons/claude_agent_sdk/runtime/protocol/query_session.gd`.
- `small bug/drift`: match upstream transport shutdown behavior after stdin EOF. The local transport currently waits about 200 ms and then force-kills the CLI, while upstream now gives the process a real grace period before escalating to terminate/kill so transcript/session-file flushes do not lose the last assistant turn. Sources: upstream `src/claude_agent_sdk/_internal/transport/subprocess_cli.py`, `tests/test_transport.py`; local `addons/claude_agent_sdk/runtime/transport/subprocess_cli_transport.gd`.
- `partially implemented`: introduce typed GDScript models for `get_context_usage()` and `get_mcp_status()` responses and lift the adapter/node/panel surfaces off raw `Dictionary` payloads. Sources: upstream `src/claude_agent_sdk/types.py` (`ContextUsageResponse`, `McpStatusResponse` and related MCP/context types); local `addons/claude_agent_sdk/runtime/claude_sdk_client.gd`, `addons/claude_agent_sdk/runtime/adapters/claude_client_adapter.gd`, `addons/claude_agent_sdk/runtime/adapters/claude_client_node.gd`, `addons/claude_agent_sdk/ui/claude_chat_panel.gd`.
...
- `small bug/drift`: make malformed known message payloads fail loudly instead of being silently dropped. Upstream raises `MessageParseError` when a recognized message type is missing required fields, but the local parser currently returns `null` after `push_error()`, which means corrupted system/rate-limit payloads can disappear from the stream without turning into a session error. Sources: upstream `src/claude_agent_sdk/_internal/message_parser.py`, `tests/test_message_parser.py`; local `addons/claude_agent_sdk/runtime/parser/message_parser.gd`, `addons/claude_agent_sdk/runtime/protocol/query_session.gd`.
- `partially implemented`: expand session-tag sanitization to full iterative upstream Unicode normalization parity. The current GDScript helper covers the common zero-width and compatibility-character cases exercised in this repo, but it is not a full port of upstream `_sanitize_unicode()`. Sources: upstream `src/claude_agent_sdk/_internal/session_mutations.py`, `tests/test_session_mutations.py`; local `addons/claude_agent_sdk/runtime/sessions/claude_sessions.gd`, `tests/runtime/ClaudeSessionsTest.gd`, `docs/contributing/session-history.md`.

### Validation and docs follow-ups

- `missing completely`: extend scripted live-CLI parity validation beyond `baseline`, `structured`, and `partial` so the delivered post-v1 slices have real-authenticated coverage too. Prioritize agents / `setting_sources`, SDK MCP tools, hook and tool-permission flows, interrupt / permission-mode / model dynamic control, stderr diagnostics, session forking, rewind/task control, and context/MCP diagnostics; cover `plugins` and `user` where feasible for the host platform. Sources: upstream `e2e-tests/test_agents_and_settings.py`, `e2e-tests/test_dynamic_control.py`, `e2e-tests/test_hook_events.py`, `e2e-tests/test_hooks.py`, `e2e-tests/test_sdk_mcp_tools.py`, `e2e-tests/test_stderr_callback.py`, `e2e-tests/test_tool_permissions.py`; local `tools/release/validate_live_cli.sh`, `docs/investigations/phase-5-validation.md`, `docs/investigations/phase-7-validation.md`, `docs/investigations/phase-8-validation.md`.
- `missing completely`: add advanced local example scripts for the post-v1 parity surface. Upstream ships concrete examples for agents, setting sources, plugins, stderr diagnostics, partial messages, hooks, budgets, and SDK MCP tooling, while the local repo currently ships only adapter/node basics under `tools/examples/`. Sources: upstream `examples/`; local `tools/examples/README.md`, `tools/examples/adapter_usage_example.gd`, `tools/examples/node_usage_example.gd`.
- `small bug/drift`: audit and correct stale parity wording in repo docs, starting with `docs/contributing/session-history.md`, which still says broader settings and agent-definition parity is deferred even though those slices are already marked delivered in the parity ledger and README.
