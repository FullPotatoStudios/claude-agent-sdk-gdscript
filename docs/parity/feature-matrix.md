...
| Public custom transport injection | `query.py`, `client.py` | Helps advanced embedders and parity testing | transport abstraction | `v1 later` | Available on the local surface through `ClaudeQuery.query(prompt, options, transport)`, `ClaudeQuery.get_auth_status(options, transport)`, and `ClaudeSDKClient.new(options, transport)`; the fake-transport runtime suites depend on it for parity and regression coverage |
...
| `get_context_usage()` | `client.py` | Valuable for advanced UIs and diagnostics | control protocol, typed response models | `v1 later` | Delivered post-v1 on the runtime surfaces, with reference-panel context diagnostics added in Phase 10Z and typed `ClaudeContextUsageResponse` parity added in Phase 10AG |
| `get_mcp_status()` / reconnect / toggle | `client.py`, `e2e-tests/test_sdk_mcp_tools.py` | Important once MCP parity matters | control protocol, MCP config, typed responses | `v1 later` | Delivered post-v1 on the runtime surfaces, with reference-panel MCP status/reconnect/toggle controls added in Phase 10Z and typed `ClaudeMcpStatusResponse` parity added in Phase 10AG |
...
| Host environment inheritance with extra env overrides | `ClaudeAgentOptions.env`, subprocess transport, sibling project reference | Needed to reuse installed Claude auth and shell setup | subprocess transport | `v1 core` | Default should inherit host env; explicit `env` adds overrides. The subprocess transport now mirrors upstream `bbec84d` W3C trace env outcomes through a Godot-native `ClaudeSubprocessCLITransport.set_trace_context_provider()` shim plus ambient env passthrough, including the POSIX `sudo` wrapper path, while still keeping this as transport-owned behavior rather than a direct OpenTelemetry dependency. |
...
| String-prompt stdin lifetime rules | `tests/test_query.py` | Prevents deadlocks around late control requests | initialize flow, result tracking | `v1 core` | Important for hooks/MCP compatibility even before those features ship; Phase 10AE delivered upstream-style shutdown grace after stdin EOF, and the runtime now models upstream-style `end_input()` timing for one-shot string/prompt-stream flows, but the shipped subprocess transport still cannot perform a true stdin half-close behind Godot's single read/write `stdio` pipe |
| CLI discovery fallback search | `_internal/transport/subprocess_cli.py` | Nice UX, but not essential if `claude` on `PATH` plus explicit path works | subprocess transport | `v1 later` | Delivered post-v1 in Phase 10Q through upstream-style fallback search for default `cli_path`, covering `PATH` plus the same common local install locations while keeping explicit custom paths untouched |
...
| Strict MCP config flag (`--strict-mcp-config`) | `_internal/transport/subprocess_cli.py`, `types.py`, upstream Python `0.1.74` | Lets callers restrict MCP servers to only those passed via `mcp_servers`, ignoring any from settings/config files | subprocess transport, MCP config | `v1 later` | Delivered post-v1 through transport-first `ClaudeAgentOptions.strict_mcp_config` forwarded as `--strict-mcp-config`, mirroring upstream Python `0.1.74` |
...
| `StreamEvent` partial-message model | parser, `e2e-tests/test_include_partial_messages.py` | Needed for token-by-token / delta UIs | parser, partial-message option | `v1 later` | Delivered in Phase 5 through `ClaudeStreamEvent`, parser support, and downstream runtime/UI handling for partial stream events |
...
| Context-usage control | `_internal/query.py`, `client.py` | Useful for advanced diagnostics | initialize flow, typed response | `v1 later` | Delivered in Phase 5 on the runtime surfaces; typed `ClaudeContextUsageResponse` parity landed in Phase 10AG and the reference-panel diagnostics landed in Phase 10Z |
| MCP status/reconnect/toggle controls | `_internal/query.py`, `client.py` | Needed once MCP support expands | initialize flow, MCP config | `v1 later` | Delivered in Phase 5 on the runtime surfaces; typed `ClaudeMcpStatusResponse` parity landed in Phase 10AG and the reference-panel diagnostics landed in Phase 10Z |
...
| Capability | Upstream entrypoints | Why it matters in Godot | Dependency chain | Bucket | Notes |
| --- | --- | --- | --- | --- | --- |
| Hook matcher and callback configuration | `types.py`, `_internal/query.py`, `examples/hooks.py`, `e2e-tests/test_hooks.py` | Powerful extension point for advanced apps | initialize flow, callback routing | `v1 later` | Delivered in Phase 5 through runtime hook matcher serialization and callback routing; later widened in Phase 10V/10X with additive typed callback helpers |
...
| Capability | Upstream entrypoints | Why it matters in Godot | Dependency chain | Bucket | Notes |
| --- | --- | --- | --- | --- | --- |
| External MCP server config passthrough | `types.py`, transport command building | Important once users need external tool ecosystems | transport, settings/config encoding | `v1 later` | Delivered in Phase 5 for external MCP configs, then widened in Phase 10D for mixed external plus SDK-hosted `mcp_servers` handling |
...
| Structured output (`output_format`) | `types.py`, `e2e-tests/test_structured_output.py` | Valuable for game/tool integrations consuming machine-readable results | result parsing, option encoding | `v1 later` | Delivered in Phase 5 through transport-side `--json-schema` emission without widening the initialize payload surface |
| Partial-message option (`include_partial_messages`) | `types.py`, `e2e-tests/test_include_partial_messages.py` | Needed for token-delta UIs | stream-event parsing | `v1 later` | Delivered in Phase 5 alongside `ClaudeStreamEvent` parsing and downstream runtime/UI handling |
...
| Setting-source controls | `types.py`, `examples/setting_sources.py`, `e2e-tests/test_agents_and_settings.py` | Useful for advanced config control | option encoding, CLI settings model | `v1 later` | Delivered post-v1 in Phase 10I through `ClaudeAgentOptions.setting_sources` and `--setting-sources` parity, including explicit empty-list passthrough for upstream-style source disabling |
...
- Phase 10AI adds saved-session subagent discovery/history helpers plus explicit-empty `setting_sources` passthrough parity
