...
| Host environment inheritance with extra env overrides | `ClaudeAgentOptions.env`, subprocess transport, sibling project reference | Needed to reuse installed Claude auth and shell setup | subprocess transport | `v1 core` | Default should inherit host env; explicit `env` adds overrides. The subprocess transport now forwards ambient W3C trace env keys (`TRACEPARENT`, `TRACESTATE`) through its explicit launch env wiring, including the POSIX `sudo` wrapper path, and explicit W3C overrides no longer inherit a stale opposite half. Full upstream active-OTEL injection parity is still not implemented locally. |
