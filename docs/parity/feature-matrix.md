...
| String-prompt stdin lifetime rules | `tests/test_query.py` | Prevents deadlocks around late control requests | initialize flow, result tracking | `v1 core` | Important for hooks/MCP compatibility even before those features ship; Phase 10AE delivered upstream-style shutdown grace after stdin EOF, while true `end_input` half-close remains pending behind Godot's single read/write `stdio` pipe |
