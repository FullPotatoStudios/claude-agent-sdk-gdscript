...
class_name ClaudeHookInputPostToolUse

var tool_name: String = ""
var tool_input: Dictionary = {}
var tool_response: Variant = null
...
func apply(config: Dictionary):
	...
	if config.has("tool_response") or config.has("toolResponse"):
		tool_response = ClaudeHookInput._duplicate_variant(_get_first(config, ["tool_response", "toolResponse"]))
...
func to_dict() -> Dictionary:
	...
	if tool_response != null:
		result["tool_response"] = ClaudeHookInput._duplicate_variant(tool_response)
	if tool_use_id != null:
