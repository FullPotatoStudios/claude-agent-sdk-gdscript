...
- SDK-hosted MCP/custom-tool registration stays code-driven through `ClaudeMcp` and `ClaudeAgentOptions.mcp_servers`; the panel can edit simple external `stdio` MCP config and inspect existing SDK MCP tool inventory, but it does not author tool handlers.
...
- when `can_use_tool` is configured, interactive `ClaudeSDKClient.query(String)` remains allowed, but `connect_client(prompt=String)` and one-shot `ClaudeQuery.query(String)` still require `ClaudePromptStream`.
